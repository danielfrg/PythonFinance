...
class PastEventTest(unittest.TestCase):
    ...
    def test_success(self):
        ...
        # Test: Series names
        self.assertEquals(self.evt.expected_return.name, 'Expected Return')
        self.assertEquals(self.evt.abnormal_return.name, 'Abnormal Return')
        self.assertEquals(self.evt.cumulative_abnormal_return.name, 'Cumulative Abnormal Return')
        self.assertEquals(self.evt.t_test.name, 't test')
        self.assertEquals(self.evt.prob.name, 'Probability')
        # Test: Index values
        self.assertEquals(self.evt.expected_return.index[0].to_pydatetime(), datetime(2008, 12, 18))
        self.assertEquals(self.evt.expected_return.index[-1].to_pydatetime(), datetime(2009, 1, 20))
        ...
        np_test.assert_almost_equal(self.evt.expected_return.values, np.array(ans_er), 3)
        np_test.assert_almost_equal(self.evt.abnormal_return.values, np.array(ans_ar), 3)
        np_test.assert_almost_equal(self.evt.cumulative_abnormal_return.values, np.array(ans_car), 2)
